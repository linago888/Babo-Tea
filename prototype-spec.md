...
- 新增 / 編輯品牌、城市、飲品、店鋪、母公司
- 發布新聞 + 來源（`sources`）管理
- 設定關聯資料（含 `relevance`：primary / secondary / mentioned）
- 維護 `taxonomies`（茶底、奶種、配料、甜味劑、定位標籤）
- 編輯 SEO title / description / FAQ（依 locale 多語）
- 草稿、發布、封存狀態（`status` enum）
- AI 摘要審核流（見 §8.1）
...
- 內部連結 ≥ 3 條（用於 SEO 內部連結密度，編輯器自動偵測 markdown 連結）
- JSON-LD 必填欄位齊全（依模板對應 schema.org type）

內容品質儀表板（dashboard 用，前台不顯示）：

- 各實體 `completeness_score` 分佈
- `orphan = true` 清單（無任何關聯的資料）
...
- `index.html`：網站 prototype 入口
- `styles.css`：響應式介面樣式
- `app.js`：搜尋、篩選、詳情面板、圖譜高亮與 Canvas 視覺
...
> 詳細逐日建表順序見 [`data-model.md`](data-model.md) §13。
...
- AI 摘要審核流上線（欄位已預埋）
- 進階搜尋（Meilisearch / Typesense 自架）
- 每週電子報
- `trending_score` / `market_maturity` 計算 job 上線（公式見 data-model §7.2）
- 城市市場比較頁
- 品牌認領（`brands.claimed_by_user_id` / `verified`）

3-12 個月：

- 全球珍奶地圖（`stores` × `cities.lat/lng`）
- AI 問答助手（以結構化資料為 RAG 來源）
- 加盟商情報（`brand_company` × `stores` × `metrics_daily`）
- 市場報告
- 企業資料 API（`slug` 與 UUID 雙鍵已備）
