...
export default async function AdminCitiesPage({
  ...
  const tStatus = await getTranslations({ locale, namespace: "admin.cities.status" });
  const tMaturity = await getTranslations({ locale, namespace: "admin.cities.marketMaturity" });
  ...
  const filtered = q
    ? cities.filter((c) => {
        ...
        return name.toLowerCase().includes(needle) || c.slug.toLowerCase().includes(needle);
      })
    : cities;
        ...
        <table className="w-full text-sm">
          <thead className="bg-neutral-50 text-xs uppercase tracking-wider text-neutral-500 dark:bg-neutral-900 dark:text-neutral-400">
            <tr>
              ...
              <th className="px-4 py-3 text-left font-semibold">{t("table.region")}</th>
              <th className="px-4 py-3 text-left font-semibold">{t("table.timezone")}</th>
              <th className="px-4 py-3 text-left font-semibold">{t("table.maturity")}</th>
              ...
              return (
                <tr key={c.id} className="transition hover:bg-neutral-50 dark:hover:bg-neutral-800">
                  ...
                  <td className="px-4 py-3 text-neutral-600 dark:text-neutral-400">
                    {localizeCountry(c.countryCode, locale)}
                  </td>
                  <td className="px-4 py-3 text-neutral-600 dark:text-neutral-400">
                    {c.adminRegion ?? "—"}
                  </td>
                  ...
                  <td className="px-4 py-3 text-neutral-600 dark:text-neutral-400">
                    {c.marketMaturity ? tMaturity(c.marketMaturity.toLowerCase()) : "—"}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-right text-xs text-neutral-500 dark:text-neutral-500">
                    {formatDate(c.updatedAt, locale, { dateStyle: "short" })}
                  </td>
                </tr>
              );
            })}
