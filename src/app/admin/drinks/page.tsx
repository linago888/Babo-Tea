...
export default async function AdminDrinksPage({
  ...
  const t = await getTranslations({ locale, namespace: "admin.drinks" });
  const tStatus = await getTranslations({ locale, namespace: "admin.drinks.status" });
  const tCategory = await getTranslations({ locale, namespace: "admin.drinks.category" });
  const tTemp = await getTranslations({ locale, namespace: "admin.drinks.temperature" });
  ...
  if (
    categoryFilter &&
    ["MILK_TEA", "FRUIT_TEA", "PURE_TEA", "CHEESE_TEA", "COFFEE_TEA", "SMOOTHIE", "OTHER"].includes(
      categoryFilter,
    )
  ) {
    where.category = categoryFilter as Prisma.DrinkWhereInput["category"];
  }
  if (q) {
  ...
  const filtered = q
    ? drinks.filter((d) => {
        const name = pickI18n(d.nameI18n, locale);
        const needle = q.toLowerCase();
        return name.toLowerCase().includes(needle) || d.slug.toLowerCase().includes(needle);
      })
    : drinks;
    ...
    <>
      ...
      <form className="mb-4 flex flex-wrap items-center gap-2" method="get">
        ...
        >
          ...
          {["MILK_TEA", "FRUIT_TEA", "PURE_TEA", "CHEESE_TEA", "COFFEE_TEA", "SMOOTHIE", "OTHER"].map((c) => (
        ...
        <table className="w-full text-sm">
          <thead className="bg-neutral-50 text-xs uppercase tracking-wider text-neutral-500 dark:bg-neutral-900 dark:text-neutral-400">
            <tr>
              ...
              <th className="px-4 py-3 text-left font-semibold">{t("table.teaBase")}</th>
              <th className="px-4 py-3 text-left font-semibold">{t("table.temperature")}</th>
              <th className="px-4 py-3 text-left font-semibold">{t("table.sweetener")}</th>
          ...
          <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
            {filtered.map((d) => {
              const name = pickI18n(d.nameI18n, locale, { fallback: d.slug });
              return (
                <tr key={d.id} className="transition hover:bg-neutral-50 dark:hover:bg-neutral-800">
                  <td className="px-4 py-3">
                    ...
                    <p className="text-xs text-neutral-500 dark:text-neutral-500">{d.slug}</p>
                  </td>
                  <td className="px-4 py-3 text-neutral-600 dark:text-neutral-400">
                    {tCategory(d.category.toLowerCase())}
                  </td>
                  <td className="px-4 py-3 font-mono text-xs text-neutral-600 dark:text-neutral-400">
                    {d.teaBase.length > 0 ? d.teaBase.join(", ") : "—"}
                  </td>
                  <td className="px-4 py-3 text-xs text-neutral-600 dark:text-neutral-400">
                    {d.temperature.length > 0
                      ? d.temperature.map((t) => tTemp(t.toLowerCase())).join(" / ")
                      : "—"}
                  </td>
                  <td className="px-4 py-3 font-mono text-xs text-neutral-600 dark:text-neutral-400">
                    {d.sweetener ?? "—"}
                  ...
                  <td className="px-4 py-3">
                    <StatusPill status={d.status} label={tStatus(d.status.toLowerCase())} />
                  </td>
                  <td className="px-4 py-3 text-right">
                    <CompletenessBadge score={d.completenessScore} />
                  </td>
                  <td className="px-4 py-3 text-right text-xs text-neutral-500 dark:text-neutral-500">
                    {formatDate(d.updatedAt, locale, { dateStyle: "short" })}
                  </td>
                </tr>
              );
            })}
